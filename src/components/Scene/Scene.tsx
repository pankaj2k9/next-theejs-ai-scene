...
function Scene({ onProgress }: {onProgress: (arg0: number) => void}) {
  ...
  useEffect(() => {
    ...
    async function loadSplatObjects() {
      const splatScene:any = new SPLAT.Scene();
      const url =
      ...
      try {
        ...
        const splatObject:any = await SPLAT.PLYLoader.LoadAsync(url, splatScene, (progress) => {
          if (isMounted) {
